...
# Media files (User uploads) - default local path
MEDIA_URL = '/media/'
MEDIA_ROOT = os.path.join(BASE_DIR, 'media')

# Use Cloudinary for image hosting when enabled
...
if USE_CLOUDINARY:
    ...
    CLOUDINARY_FOLDER = config('CLOUDINARY_FOLDER', default='booking_app')
    ...
    # Use Cloudinary storage - Django 5.0+ STORAGES format
    STORAGES = {
        "default": {
    ...
    # Also set legacy setting for backward compatibility
    DEFAULT_FILE_STORAGE = 'utils.cloudinary_storage.CloudinaryMediaStorage'
    MEDIA_URL = f'https://res.cloudinary.com/{CLOUDINARY_CLOUD_NAME}/image/auto/'
    
    # Debug output
    if DEBUG:
        print("[storage-debug] Using Cloudinary for media storage")
        print("[storage-debug] CLOUDINARY_CLOUD_NAME:", CLOUDINARY_CLOUD_NAME)
        print("[storage-debug] CLOUDINARY_FOLDER:", CLOUDINARY_FOLDER)
        print("[storage-debug] DEFAULT_FILE_STORAGE:", DEFAULT_FILE_STORAGE)

elif config('USE_DO_SPACES', default=False, cast=bool):
    # Legacy DigitalOcean Spaces support (disabled when using Cloudinary)
    # Map DigitalOcean Spaces credentials
    ...
    DO_SPACES_CUSTOM_DOMAIN = config('DO_SPACES_CUSTOM_DOMAIN', default='')
    DO_SPACES_ENDPOINT_URL = config(
        'DO_SPACES_ENDPOINT_URL',
        default=f"https://{DO_SPACES_REGION}.digitaloceanspaces.com",
    )

    # django-storages expects AWS-style names
    ...
    # Storage settings
    AWS_DEFAULT_ACL = 'public-read'  # Set to 'public-read' for public access or None for private
    AWS_QUERYSTRING_AUTH = False  # Don't add auth query params to URLs
    AWS_S3_FILE_OVERWRITE = False  # Don't overwrite files with same name
    AWS_LOCATION = 'media'  # Optional: folder within bucket
    
    # Cache and expiration settings (use callable or remove)
    AWS_S3_OBJECT_PARAMETERS = {
        'CacheControl': 'max-age=86400',
    }

    # Use custom DigitalOcean Spaces storage backend
    STORAGES = {
        "default": {
            "BACKEND": "utils.storage.DigitalOceanSpacesStorage",
        },
        "staticfiles": {
            "BACKEND": "whitenoise.storage.CompressedManifestStaticFilesStorage",
        },
    ...
    DEFAULT_FILE_STORAGE = 'utils.storage.DigitalOceanSpacesStorage'
    MEDIA_URL = f"https://{AWS_S3_CUSTOM_DOMAIN}/"
    
    # If using AWS_LOCATION, append it to MEDIA_URL
    if AWS_LOCATION:
        MEDIA_URL = f"https://{AWS_S3_CUSTOM_DOMAIN}/{AWS_LOCATION}/"
        
    if DEBUG:
        print("[storage-debug] Using DigitalOcean Spaces for media storage")
        print("[storage-debug] DO_SPACES_BUCKET_NAME:", DO_SPACES_BUCKET_NAME)
        print("[storage-debug] DEFAULT_FILE_STORAGE:", DEFAULT_FILE_STORAGE)
else:
    ...
    if DEBUG:
        print("[storage-debug] Using local/database storage")
        print("[storage-debug] DEFAULT_FILE_STORAGE:", DEFAULT_FILE_STORAGE)

# Ensure the media directory exists for local/database storage
...
if DEBUG:
    # Storage debug output is now handled in the storage configuration above
    pass
